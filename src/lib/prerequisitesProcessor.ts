...
export const getPrerequisiteList = (
  variant: Variant,
): NewPrerequisiteType[] => {
  let output: NewPrerequisiteType[] = [];

  const cardsAndTemplates: Array<
    (CardComponent | Template) & { name: string }
  > = (variant.uses as Array<CardComponent | Template>)
  ...
  const cardNameCountMap = cardsAndTemplates.reduce(
    (acc: Record<string, number>, card) => {
      const split =
        'card' in card ? card.card.name.split(', ')[0] : card.template.name;
  ...
  const cardNameMap = cardsAndTemplates.reduce(
  ...
  const multiZoneCards = cardsAndTemplates.filter(
    (card) => card.zoneLocations.length > 1,
  );
  for (const card of multiZoneCards.sort((a, b) =>
    ...
    if (Object.keys(ZONE_MAP).length === card.zoneLocations.length) {
      ...
      cardString += card.zoneLocations
        .map((zone) => ZONE_MAP[zone as keyof typeof ZONE_MAP])
        .join(' or ');
    }
    const combinedStateString = comaAndOrJoin(
      Object.values(getZoneStateMap(card)),
    );
    if (combinedStateString) cardString += ` (${combinedStateString})`;
    cardString += '. ';
    output.push({ z: 'multi', s: cardString });
  }
  const singleZoneCards = cardsAndTemplates.filter(
    (card) => card.zoneLocations.length === 1,
  );
  ...
  for (const zoneKey in ZONE_MAP) {
    const zoneCards = singleZoneCards.filter(
      (card) => card.zoneLocations[0] === zoneKey,
    );
    ...
    const stateMap = zoneCards.reduce((acc: Record<string, string[]>, card) => {
      ...
      if (cardState)
        acc[cardState] = acc[cardState]
          ? acc[cardState].concat([cardNameMap[card.name]])
          : [cardNameMap[card.name]];
      return acc;
    }, {});
    console.log(stateMap); // TODO: Remove
  ...
  let index = 0;
  for (const zoneGroup of zoneGroups.sort((a, b) =>
    a.cardNames.length > b.cardNames.length ? 1 : -1,
  )) {
    ...
    if (index === zoneGroups.length - 1 && cards.length > 2) {
      ...
      output.push({
        ...
        s:
          (cards.length < 3
            ? cards.join(' and ')
            : cards.slice(0, -1).join(', ') + ' and ' + cards.slice(-1)) +
          ' ' +
  ...
  if (variant.otherPrerequisites)
    variant.otherPrerequisites
      .split(/\.\s+/gi)
      .forEach((prereq) => output.push({ z: 'other', s: prereq }));
