    ...
    <PageWrapper>
      ...
      <div className={styles.syntaxGuideContainer}>
        ...
        <div className={styles.searchGuideContainer}>
          ...
          >
            <p>
              You can find cards in a combo simply by entering the card names,
              or parts of the card names, into the search bar. Passing multiple
              card names will result in combos that have cards that contain
              those words among the names of the cards in the combo.
            </p>
            <p>
              You can lookup longer names for cards by using the &nbsp;
              <code>card</code> key with single (<code>'</code>) or double (
              <code>"</code>) quotes. This will find combos that contain cards
              with names that include the whole phrase. For instance, &nbsp;
              <code>card:dream</code>, will include combos that contain the card
              Sickening Dreams, Dream Stalker, Dreamtail Heron, etc. If &nbsp;
              <code>card:"dream stalk"</code> is used, then only combos that
              contain Dream Stalker wil lbe displayed.
            </p>
            <p>
            ...
            <p>
              Use <code>-card</code> To find combos that do not include a
              certain card.
            </p>
            <p>
              The <code>=</code>, <code>&gt;</code>, <code>&lt;</code>, &nbsp;
              <code>&gt;=</code>, <code>&lt;=</code> operators can also be used
              with numbers to to find combos restricted by the number of cards.
            </p>

            <p>
              An alias for <code>card</code> is <code>cards</code>.
            </p>
          </SearchGuide>
          ...
          >
            <p>
              ...
              <code>coloridentity:bug</code>, which in this example finds all
              the combos that can be played within a green, blue, black color
              identity.
            </p>

            <p>
              The <code>coloridentity</code> parameter accepts full color names
              like &nbsp;<code>green</code> or the single character
              representations of the colors (<code>w</code>, <code>u</code>,{" "}
              <code>b</code>, <code>r</code>, &nbsp;<code>g</code>). You can use
              the names for color combinations such as &nbsp;<code>boros</code>,{" "}
              <code>izzet</code>, <code>naya</code>, &nbsp;<code>sultai</code>,
              etc.
            </p>
            ...
            <p>
              The <code>=</code>, <code>&gt;</code>, <code>&lt;</code>, &nbsp;
              <code>&gt;=</code>, <code>&lt;=</code> operators can also be used
              to find combos with a range of color identities. These operators
              can be used with the color values or a number to indicate the
              number of colors required for the combo.
            </p>

            <p>
              Aliases for the <code>coloridentity</code> key include &nbsp;
              <code>color_identity</code>, <code>color</code>,{" "}
              <code>colors</code>, &nbsp;<code>commander</code>, <code>id</code>
              , <code>ids</code>, &nbsp;<code>c</code>, and <code>ci</code>.
            </p>
          </SearchGuide>
          ...
          >
            <p>
              The prerequisites are the elements that need to be in place before
              the combo can be initiated.
            </p>

            <p>
              You can also use <code>-prerequisites</code> to find combos that
              do not contain the value passed.
            </p>

            <p>
              The <code>=</code>, <code>&gt;</code>, <code>&lt;</code>, &nbsp;
              <code>&gt;=</code>, <code>&lt;=</code> operators can also be used
              with numbers to to find combos restricted by the number of
              prerequisites.
            </p>

            <p>
              Aliases for <code>prerequisites</code> are{" "}
              <code>prerequisite</code> and &nbsp;<code>pre</code>.
            </p>
          </SearchGuide>
          ...
          >
            ...
            <p>
              You can also use <code>-steps</code> to find combos that do not
              contain steps with the value passed.
            </p>

            <p>
              The <code>=</code>, <code>&gt;</code>, <code>&lt;</code>, &nbsp;
              <code>&gt;=</code>, <code>&lt;=</code> operators can also be used
              with numbers to to find combos restricted by the number of steps.
            </p>

            <p>
              An alias for <code>steps</code> is <code>step</code>.
            </p>
          </SearchGuide>
          ...
          >
            <p>
              The results are the effects that occur as a result of completing
              the combo.
            </p>
            ...
            <p>
              An alias for <code>results</code> is <code>result</code>.
            </p>
          </SearchGuide>
          ...
          >
            <p>
              You can also search by <code>spellbookid</code> if you want to
              find a specific combo by its id. Using more than one
            ...
            <p>
              A more useful parameter to use is <code>-spellbookid</code>, to
              more easilly omit certain combos. For instance, for finding all
              combos using Basalt Monolith and Mesmeric Orb except
          ...
          >
            <p>
              You can also search by <code>popularity</code> to find combos by
              the number of decks that contain the combo according to
              <ExternalLink href="https://edhrec.com/">EDHREC</ExternalLink>.
            </p>

            <p>
              Use the <code>=</code>, <code>&gt;</code>, <code>&lt;</code>,
              &nbsp;<code>&gt;=</code>, <code>&lt;=</code> operators when
          ...
          >
            <p>
              ...
              &nbsp;<code>price</code> to to find combos restricted by the price
              of the combo.
            </p>
            ...
            <p>
              Combos where the price is not available are omitted from search
              results when using the <code>price</code> parameter.
            </p>

            <p>
              <code>usd</code> is an alias for <code>price</code>.
            </p>
          </SearchGuide>
          ...
          <SearchGuide
            ...
            snippets={DATA.bannedSnippets}
          >
            <p>
              By default, combo results will not include combos that contain
              cards that are banned in Commander. Use{" "}
              <code>include:banned</code> to allow combo results that are banned
              in Commander.
            </p>

            <p>
              To find specific combos that contain cards that are banned in
              Commander, use <code>is:banned</code>.
            </p>
          ...
          >
            <p>
              By default, combo results will be sorted by popularity (the number
              of decks the combo is in according to
              <ExternalLink href="https://edhrec.com/">EDHREC</ExternalLink>),
              starting with combos in the most decks and going to the least. Use
              &nbsp;<code>sort</code> to change the criteria for sorting the
              combos. Available options are:
            </p>

            <ul>
              <li>
                <code>colors</code> (or <code>ci</code>,{" "}
                <code>color-identity</code>,<code>color</code>)
              </li>

              <li>
                <code>price</code> (or <code>usd</code>)
              </li>

              <li>
                <code>results</code> (or <code>number-of-results</code>)
              </li>
              <li>
                <code>steps</code> (or <code>number-of-steps</code>)
              </li>

              <li>
                <code>prerequisites</code> (or{" "}
                <code>number-of-prerequisites</code>)
              </li>

              <li>
                <code>cards</code> (or <code>number-of-cards</code>)
              </li>
            ...
            <p>
              By default, when sorted by popularity, the combo results are
              ordered in descending order. All other sort options order the
              results in ascending order instead. Use
              <code>order</code> to alter the default behavior.
            </p>

            <p>
              When sorting by price, combos with no price available are treated
              as having a price of 0.
            </p>
          </SearchGuide>
        </div>
      </div>
