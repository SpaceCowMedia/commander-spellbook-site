...
const AutocompleteInput: React.FC<Props> = ({value, inputClassName, autocompleteOptions, inputId, label, matchAgainstOptionLabel, useValueForInput, placeholder, hasError, onChange }: Props) => {

  const resultsRef = React.useRef<HTMLUListElement>(null)
  const inputRef = React.useRef<HTMLInputElement>(null)
  const timeoutRef = React.useRef<ReturnType<typeof setTimeout>>()
  const [localValue, setLocalValue] = useState<string>(value)
  const [matchingAutoCompleteOptions, setMatchingAutoCompleteOptions] = useState<Array<{value: string, label: string}>>([])
  ...
  if (total !== 0 && value) screenReaderSelectionText = option  ? `${option.label} (${arrowCounter + 1}/${total})` : `${total} match${total > 1 ? "es" : ""} found for ${value}. Use the up and down arrow keys to browse the options. Use the enter or tab key to choose a selection or continue typing to narrow down the options.`
  ...
  const findAllMatches = (normalizedValue: string) => autocompleteOptions.filter((option) => {
  ...
  const createAutocompleteTimeout = () => setTimeout(() => {
    if (!value) return close()
    ...
    setMatchingAutoCompleteOptions(findBestMatches(
      totalOptions,
      normalizedValue
    ))
  ...
  return (
    <div className={styles.autocompleteContainer}>
      <label className="sr-only" aria-hidden htmlFor={inputId}>{label}</label>
      <input
        id={inputId}
        ref={inputRef}
        value={localValue}
