...
type MultiSearchInputValue = { value: string; operator: string, error?: string }[]
...
type Props = {
  ...
  operatorOptions: Array<{value: string, label: string, placeholder?: string}>
  defaultOperator?: string
  onChange?: (value: MultiSearchInputValue) => void
}

const MultiSearchInput = ({ value, autocompleteOptions = [], useValueForAutocompleteInput, label, pluralLabel, defaultPlaceholder, operatorOptions, defaultOperator = '', onChange }: Props) => {

  const [inputs, setInputs] = useState<MultiSearchInputValue>(value)

  const inputLabel =  pluralLabel || pluralize(label, value.length)

  const addInput = (index: number) => setInputs([...inputs.slice(0, index + 1), { value: '', operator: defaultOperator }, ...inputs.slice(index + 1)])

  const removeInput = (index: number) => setInputs([...inputs.slice(0, index), ...inputs.slice(index + 1)])
        ...
        <div key={`${label}-input-${index}`} className={`my-2 input-wrapper-${index}`}>
          <div className="sm:flex">
            <StyledSelect
              ...
              selectBackgroundClassName={`${input.error ? 'border-danger' : 'border-dark'} border border-b-0 sm:border-b sm:border-r-0 sm:w-1/2 flex-grow`}
            />
            <div className="w-full flex-grow flex flex-col sm:flex-row">
              ...
              <div className="flex">
                {inputs.length > 1 && (
                  <button
                    type="button"
                    className={`minus-button ${styles.inputButton} ${input.error ? 'bg-danger border-danger' : 'bg-dark border-dark'} minus-button-${index}`}
                    onClick={() => removeInput(index)}
                    >
                    –
                    <span className="sr-only">Remove {input.value} search query for {label}</span>
                    <span aria-hidden="true"/>
                  </button>
                )}
                <button
                  type="button"
                  className={`plus-button ${styles.inputButton} ${input.error ? 'bg-danger border-danger' : 'bg-dark border-dark'} plus-button-${index}`}
          ...
          {input.error && <div className={`input-error text-danger w-full py-2 px-4 text-center rounded-b-sm`}>{input.error}</div>}
        </div>
      ))}
    </div>
  )
